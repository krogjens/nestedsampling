function [walker_new,delta_a,delta_r]=ns_join_walker_evolve(obs,model,logLstar,walker,delta_u,u_unfix,model1,walker_step)
...
delta_a=zeros(size(walker));;
delta_r=zeros(size(walker));;
...
if lenobs>1 || sum(u_unfix)>0
  ...
  for n=1:lenobs
    ...
    [walker1_new,delta_a1,delta_r1]=walker_step(obs{n},model1,logLstar1,walker1,delta_u1);
