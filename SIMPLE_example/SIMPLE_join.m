...
lenobs=5;
for n=1:lenobs
  data{n}=cumsum(sqrt(2*1)*randn(1000,1));
end

models_join(1)=ns_join_simple(models(1),1);

u_unfix=[0 1];
theta_unfix=u_unfix;
add_unfix=[];
models_join(2)=ns_join_hetero(models(2),u_unfix,theta_unfix,add_unfix,lenobs);

%Run the nested sampling algorithm for all models and compile results
[results] = ns_processdataset(data,models_join,misc);
