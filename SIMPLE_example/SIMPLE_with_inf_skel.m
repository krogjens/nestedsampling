...
options.nlist = 1:1:10;    % Maximum time scaling factor for data
...
%Specify the function that subsamples the data
models(1).scaling =@(obs,n) obs(1:n:end); 
models(2).scaling =@(obs,n) obs(1:n:end);

%Specify the functions that replicate data
models(1).replicate =@(obs,theta,n) cumsum(sqrt(2*theta(1)*n)*randn(size(obs))); 
models(2).replicate =@(obs,theta,n) cumsum(n*theta(2)+sqrt(2*theta(1)*n)*randn(size(obs))); 
...
misc.data_id = 'simple';
data=models(1).replicate(1:1000,1,1);
